...
#Teste de correlaçao de phenotype.
FREQ_PHENO_CYP2C19 <- read_xlsx(choose.files(), sheet = "FREQ_FENOTIPICA")
...
for (frequencia in phenotype) {
  Freq_column <- as.numeric(FREQ_PHENO_CYP2C19[[frequencia]])
...
FREQ_SCORE_CYP2C19 <- read_xlsx(choose.files(), sheet = "FREQ_SCORE")
...
for (frequencia in score) {
  Freq_column <- as.numeric(FREQ_SCORE_CYP2C19[[frequencia]])
  ...
  for (coluna in colunas) {
    ...
    resultado_df_score <- rbind(resultado_df_score, data.frame(
      Frequencia = frequencia,
      Coluna = coluna,
      Valor_p = cor_test_2$p.value,
      Valor_r = cor_test_2$estimate ))
...
library(clipr)
write_clip(resultado_df_score, dec = ",")